...
class Game : public Entity {
  ...
  public:
	...
	void enter() override { 
		...
		std::unique_ptr<Bike> b = std::make_unique<BikePlayer>(window, render, 1, car_model, ai_waypoints[0], wm.get());
		...
		for (int i = 0; i < players - 1; i++) {
			...
			std::unique_ptr<Bike> b = std::make_unique<BikeAI>(render, i+2, car_model, ai_waypoints[0], wm.get());
			bikes.push_back(b.get());
